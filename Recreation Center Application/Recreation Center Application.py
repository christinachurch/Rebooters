...
class RecreationCenterApplication:
    ...
    def waterRental(self):
        ...
        tk.Button(page, text='Calculate', command=self.calculateLandCost(self.tent_var.get(), self.sleepingbag_var.get(), self.backpack_var.get(), self.twoburnerstove_var.get(), self.lantern_var.get(), self.cookpotset_var.get(), self.watercooler_var.get(), self.hammock_var.get())).grid(row=11, column=2)
    ...
    def transactionSummaryPage(self):
        page = tk.Frame(self.master)
        page.grid()
        tk.Label(page, text='Transaction Summary').grid(row=0)

...
# Function to calcuate the final cost of the equipment.
    def calculateLandCost(self, tent, sleepingbag, backpack, twoburnerstove, lantern, cookpotset, hammock, watercooler):
        if self.delta > 3:
            for key in landEquipment.keys():
                landEquipment[key] = landEquipment[key] * 2
    ...
    def calculateWaterCost(self, canoe, kayak, paddleboard, pfd, paddle, drybag, drypack):
        if self.delta > 3:
            for key in waterEquipment.keys():
                waterEquipment[key] = waterEquipment[key] * 2
...
# Function creates the first page to select the date and type of rental.
    def dateSelection(self):
        ...
        self.startDate = tkc.DateEntry(page, textvariable = self.start, mindate = today, maxdate = today + timedelta(days=7), width=15).grid(row=2, column=1, pady= 10, sticky=tk.W)

        self.end = tk.StringVar()
        tk.Label(page, text="End Date:", font=('Arial', 14), fg='white', bg="#367653").grid(row=3, column=0,pady= 10, sticky=tk.W)
        self.endDate = tkc.DateEntry(page, textvariable = self.end, width=15, mindate = today, maxdate = today + timedelta(days=14)).grid(row=3, column=1, pady= 10, sticky=tk.W)
        ...
        tk.Button(page, text='Next Page', command=lambda : self.confirmDate(self.start, self.end)).grid(row=9, pady= 10)
...
# Function creates the second page to select the equipment for land rentals.
    ...
    def landRental(self):
        ...
        self.calcLandTotal = lambda: [self.calculateLandCost(self.tent_var.get(), self.sleepingbag_var.get(), self.backpack_var.get(), self.twoburnerstove_var.get(), self.lantern_var.get(), self.cookpotset_var.get(), self.watercooler_var.get(), self.hammock_var.get()), self.goto_page3()]
        tk.Button(page, text='Back', command=self.goto_page1).grid(row=11)
        tk.Button(page, text='Next Page', command=self.calcLandTotal).grid(row=11, column=1)
...
# Function that creates the water rental page
    def waterRental(self):
        ...
        self.calcWaterTotal = lambda : [self.calculateWaterCost(self.canoe_var.get(), self.kayak_var.get(), self.paddleboard_var.get(), self.pfd_var.get(), self.paddles_var.get(), self.drybag_var.get(), self.drypack_var.get()), self.goto_page3()]
        tk.Button(page, text='Back', command=self.goto_page1).grid(row=11)
        tk.Button(page, text='Next Page', command=self.calcWaterTotal).grid(row=11, column=1)
...
# Function that creates the transaction summary page
    def transactionSummaryPage(self):
        ...
        tk.Label(page, text=f'Start Date: {self.start.get()}').grid(row=rownum, column=1)
        tk.Label(page, text=f'End Date: {self.end.get()}').grid(row=rownum, column=2)
        tk.Button(page, text='Submit', command=self.confirmOrder).grid(row=rownum+1, column=1)
...
# Function that moves from land or water rental page to transaction summary page of the water rental page, if both land and water are selected
    ...
    def confirmOrder(self):
        tk.Label(self.master, text='Order Confirmed!', fg='green').grid(row=3, column=0)

#used to confirm that the dates chosen on page 1 are valid
    def confirmDate(self, start, end):
        startDay = datetime.strptime(start.get(),'%m/%d/%y')
        endDay = datetime.strptime(end.get(),'%m/%d/%y')

        self.delta = endDay - startDay
        self.delta = self.delta.days

        if self.delta < 1:
            tk.Label(self.master, text='Invalid Dates - End date must be after Start Date', fg='red').grid(row=3, column=0)
        elif self.delta > 7:
            tk.Label(self.master, text='Invalid Dates - Rental period cannot exceed 7 days', fg='red').grid(row=3, column=0)
        else:
